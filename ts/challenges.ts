...
let challenges: {[key: string]: t_chal} = {
  c1: {
    name: "challenge I",
    desc: () => `forager bees consume ${3 ** (1 + p.challengeCompletions.c1)}x more flowers`,
    effect: () => `1.04x forager bee production per log${Math.min(8, 9 - p.challengeCompletions.c1)} of flowers`,
    eff: (x) => format(x) + "x",
  },
  c2: {
    name: "challenge II",
    desc: () => `${3 ** (p.challengeCompletions.c2 + 1)}x less worker space for bees`,
...
const setupChallenges = () => {
  ...
  Object.keys(challenges).forEach((key, i) => {
    if (i <= 4) $("challengesContainer").innerHTML += makeChallenge(challenges[key]!, key);
    else $("challengesContainer").innerHTML += makeChallenge(challenges[key]!, key, "d");
  });
...
const makeChallenge = ({name}: t_chal, key: string, c: string = "") => {
