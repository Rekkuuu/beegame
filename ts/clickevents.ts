...
const sacrificeToGod = () => {
  ...
  freeBees += p.RJbees;
  if (tmp.totalTributes >= tributeMilestones[3]) freeBees += Math.floor(tmp.totalTributes / 5);
  tmp.m5e = Math.min(9, 1.02 ** ((tmp.totalTributes / 10) * tmp.m5e));
  tmp.m5e = Math.min(9, 1.02 ** ((tmp.totalTributes / 10) * tmp.m5e));
  tmp.m5e = Math.min(9, 1.02 ** ((tmp.totalTributes / 10) * tmp.m5e));
  if (p.honeyGodTributes + p.honeyGodRJTributes > 0) freeBees *= 1.03 ** ((p.honeyGodTributes + p.honeyGodRJTributes) * tmp.m5e);
...
type t_tabs1 = "main" | "jelly" | "help" | "settings";
let tabs1: t_tabs1[] = ["main", "jelly", "help", "settings"];
...
const exchangeProgress = () => {
  let rjToGet = RJToGet();
  if (p.exchangeConfirmation && !confirm(`are you sure you want to reset all your progress for ${format(rjToGet, 1)} royal jelly`)) return;
  p.RJ += rjToGet;
  p.totalRJ += rjToGet;
...
d.removepollenGodTribute.addEventListener("click", () => {
  if (p.pollenGodRJTributes + tmp.tmpRJpollenGodTributes <= 0) return;
  tmp.tmpRJpollenGodTributes--;
  tmp.tmpunusedRJTributes++;
});
...
d.removenectarGodTribute.addEventListener("click", () => {
  if (p.nectarGodRJTributes + tmp.tmpRJnectarGodTributes <= 0) return;
  tmp.tmpRJnectarGodTributes--;
  tmp.tmpunusedRJTributes++;
});
...
d.removehoneyGodTribute.addEventListener("click", () => {
  if (p.honeyGodRJTributes + tmp.tmpRJhoneyGodTributes <= 0) return;
  tmp.tmpRJhoneyGodTributes--;
  tmp.tmpunusedRJTributes++;
});
...
d.removeflowerGodTribute.addEventListener("click", () => {
  if (p.flowerGodRJTributes + tmp.tmpRJflowerGodTributes <= 0) return;
  tmp.tmpRJflowerGodTributes--;
  tmp.tmpunusedRJTributes++;
});
...
d.removecapitalistGodTribute.addEventListener("click", () => {
  if (p.capitalistGodRJTributes + tmp.tmpRJcapitalistGodTributes <= 0) return;
  tmp.tmpRJcapitalistGodTributes--;
  tmp.tmpunusedRJTributes++;
});
