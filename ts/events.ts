...
const assignBeesTo = (assignTo: "forager" | "honey" | "free", assignableBees: number): number => {
...
const buyHive = () => {
  if (p.pollen < getHivePrice()) return;
  p.pollen -= getHivePrice();
  p.hives++;
};
const buyFlowerField = () => {
  if (p.money < getFlowerFieldPrice()) return;
...
const sacrificeToGod = () => {
  ...
  if (n_tributes.tmp.totalTributes >= n_tributes.tributes[3].unlockAt) freeBees += n_tributes.tributes[3].formula();
  if (p.honeyGodTributes + p.honeyGodRJTributes > 0)
    freeBees *= 1.03 ** ((p.honeyGodTributes + p.honeyGodRJTributes) * n_tributes.tmp.me[5]);
  ...
  if (p.pollenGodTributes > 0) p.pge = true;
  if (p.nectarGodTributes > 0) p.nge = true;
...
const tributesFromSacrifice2 = (maxTributes: number, tributes: number, highestResource: number): number => {
...
const sacrificeTo = (gods: t_gods[]) => {
  gods.forEach((god) => {
    sac[god]();
  });
};

type t_tabs1 = "main" | "jelly" | "gods" | "statshelp" | "settings";
let tabs1: t_tabs1[] = ["main", "jelly", "gods", "statshelp", "settings"];

type t_tabs2 = "combine" | "challenges";
let tabs2: t_tabs2[] = ["combine", "challenges"];

type t_tabs3 = "stats" | "help";
let tabs3: t_tabs3[] = ["stats", "help"];

const e_switchTab1 = (tab: t_tabs1) => {
  tabs1.forEach((_tab) => (d[`${_tab}TabContent`].style.display = "none"));
  d[`${tab}TabContent`].style.display = "";
  p.tab = tab;
};

const e_switchTab2 = (tab: t_tabs2) => {
  tabs2.forEach((_tab) => (d[`${_tab}TabContent`].style.display = "none"));
  d[`${tab}TabContent`].style.display = "";
  p.tab2 = tab;
};

const e_switchTab3 = (tab: t_tabs3) => {
...
const e_buyMaxBees = () => {
  ...
  let beesLeft = bought * 1.03 ** p.honeyGodTributes;

  autoAssignBees(beesLeft);
};

const autoAssignBees = (beesLeft: number) => {
  if (p.autoAsignBeesTo[0] != undefined) beesLeft = assignBeesTo(p.autoAsignBeesTo[0], beesLeft);
  if (p.autoAsignBeesTo[1] != undefined) beesLeft = assignBeesTo(p.autoAsignBeesTo[1], beesLeft);
  assignBeesTo("free", beesLeft);
...
const e_RJsellflowerFields = () => {
  if (p.RJflowerFields <= 0) return;
  p.RJ += structurePrice(p.RJflowerFields - 1);
  p.RJflowerFields--;
};

const e_RJsellbees = () => {
  if (p.RJbees <= 0) return;
  p.RJ += structurePrice(p.RJbees - 1);
  p.RJbees--;
};

const e_RJsellhives = () => {
  if (p.RJhives <= 0) return;
...
const e_export = () => {
  let output = $("saveExport") as HTMLTextAreaElement;
...
const strAutobuyerChange = (e: Event, type: "flower" | "bee" | "hive") => {
...
d.autoflowerBuyPercent.addEventListener("input", (e) => setInputWidth(e.target as HTMLInputElement));
d.autobeeBuyPercent.addEventListener("input", (e) => setInputWidth(e.target as HTMLInputElement));
d.autohiveBuyPercent.addEventListener("input", (e) => setInputWidth(e.target as HTMLInputElement));

const blurEnter = (e: KeyboardEvent) => e.key == "Enter" && (e.target as HTMLInputElement).blur();
...
const e_enterChallenge = (key: string) => {
  p.challenge = key;
};
...
const setupClickEvents = () => {
  ...
  d.foragerbeestextunderline.addEventListener("click", () => {
    if (p.autoAsignBeesTo[0] == "forager") p.autoAsignBeesTo.shift();
    else {
  ...
  d.honeybeestextunderline.addEventListener("click", () => {
    if (p.autoAsignBeesTo[0] == "honey") p.autoAsignBeesTo.shift();
    else {
  ...
  d.sacrificeToPollenGod.addEventListener("click", () => {
    if (p.settings.sacrificeConfirmation && !confirm("are you sure you want to sacrifice to Pollen God?")) return;
    sacrificeTo(getConnectedTo("pollen"));
    sacrificeToGod();
  });
  d.sacrificeToNectarGod.addEventListener("click", () => {
    if (p.settings.sacrificeConfirmation && !confirm("are you sure you want to sacrifice to Nectar God?")) return;
    sacrificeTo(getConnectedTo("nectar"));
    sacrificeToGod();
  });
  d.sacrificeToHoneyGod.addEventListener("click", () => {
    if (p.settings.sacrificeConfirmation && !confirm("are you sure you want to sacrifice to Honey God?")) return;
    sacrificeTo(getConnectedTo("honey"));
    sacrificeToGod();
  });
  d.sacrificeToFlowerGod.addEventListener("click", () => {
    if (p.settings.sacrificeConfirmation && !confirm("are you sure you want to sacrifice to Flower God?")) return;
    sacrificeTo(getConnectedTo("flower"));
    sacrificeToGod();
  });
  d.sacrificeToCapitalistGod.addEventListener("click", () => {
    if (p.settings.sacrificeConfirmation && !confirm("are you sure you want to sacrifice to Capitalist God?")) return;
  ...
  d.autoflowerButton.addEventListener("click", () => {
    if (p.RJ < 5 || p.autobuy.structures.flower) return;
    p.autobuy.structures.flower = true;
    p.RJ -= 5;
  });
  d.autobeeButton.addEventListener("click", () => {
    if (p.RJ < 5 || p.autobuy.structures.bee) return;
  ...
  d.removepollenGodTribute.addEventListener("click", () => {
    if (p.pollenGodRJTributes + n_jelly.tmp.tmpRJpollenGodTributes <= 0) return;
    n_jelly.tmp.tmpRJpollenGodTributes--;
    n_jelly.tmp.tmpunusedRJTributes++;
  });
  ...
  d.removenectarGodTribute.addEventListener("click", () => {
    if (p.nectarGodRJTributes + n_jelly.tmp.tmpRJnectarGodTributes <= 0) return;
    n_jelly.tmp.tmpRJnectarGodTributes--;
    n_jelly.tmp.tmpunusedRJTributes++;
  });
  ...
  d.removehoneyGodTribute.addEventListener("click", () => {
    if (p.honeyGodRJTributes + n_jelly.tmp.tmpRJhoneyGodTributes <= 0) return;
    n_jelly.tmp.tmpRJhoneyGodTributes--;
    n_jelly.tmp.tmpunusedRJTributes++;
  });
  ...
  d.removeflowerGodTribute.addEventListener("click", () => {
    if (p.flowerGodRJTributes + n_jelly.tmp.tmpRJflowerGodTributes <= 0) return;
    n_jelly.tmp.tmpRJflowerGodTributes--;
    n_jelly.tmp.tmpunusedRJTributes++;
  });
  ...
  d.removecapitalistGodTribute.addEventListener("click", () => {
    if (p.capitalistGodRJTributes + n_jelly.tmp.tmpRJcapitalistGodTributes <= 0) return;
    n_jelly.tmp.tmpRJcapitalistGodTributes--;
    n_jelly.tmp.tmpunusedRJTributes++;
  });
  ...
  Object.keys(challenges).forEach((key) => {
    (d[`CH${key}Button`] as HTMLButtonElement).addEventListener("click", () => e_enterChallenge(key));
  });
