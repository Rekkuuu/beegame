...
const getBeePrice = () => {
    ...
    if (tmp.totalTributes >= tributeMilestones[1])
        price /= Math.pow(1.02, ((tmp.totalTributes * tmp.m5e) / 2));
    return ft2(price);
};
const getHivePrice = () => {
    let a = Math.pow(c, (p.hives - 1));
    let price = Math.floor(2 * a);
    price /= Math.pow(1.03, (p.capitalistGodTributes * tmp.m5e));
    return ft2(price);
};
...
const getMaxForagerBees = () => {
    ...
    space *= Math.pow(1.03, (p.capitalistGodTributes * tmp.m5e));
...
const getFlowerProduction = (flowerFields = p.flowerFields + p.flowerGodTributes * tmp.m5e) => {
    ...
    prod *= Math.pow(1.01, Math.max(0, p.flowerFields - 1 + p.flowerGodTributes * tmp.m5e));
    prod *= Math.pow(1.03, (p.flowerGodTributes * tmp.m5e));
    prod *= tmp.m0e;
...
const getPollenProduction = (foragerBees = p.foragerBees) => {
    ...
    prod *= Math.pow(1.01, Math.max(0, p.flowerFields + 2 * p.flowerGodTributes * tmp.m5e - 1));
    prod *= Math.pow(1.03, (p.pollenGodTributes * tmp.m5e));
    if (p.pge)
        prod *= 2;
    prod *= tmp.m8e;
    prod *= tmp.gameSpeedFormTicks;
...
const getNectarProduction = (foragerBees = p.foragerBees) => {
    ...
    prod *= Math.pow(1.03, (p.nectarGodTributes * tmp.m5e));
    prod *= tmp.m6e;
    prod *= tmp.m8e;
    prod *= tmp.gameSpeedFormTicks;
    return prod;
...
const updateDisplay = () => {
    ...
    if (tmp.totalTributes == 0)
        ...
        d.recomended.style.display = "none";
    ...
    if (p.capitalistGodTributes > 0 || p.highestmoney >= 35)
...
const updateText = () => {
    ...
    if (tmp.m5e == 1)
        d.flowerFields.innerHTML = "" + `${p.flowerFields.toFixed(0)} + ${p.flowerGodTributes.toFixed(0)}`;
    else
        d.flowerFields.innerHTML = "" + `${p.flowerFields.toFixed(0)} + ${format(p.flowerGodTributes * tmp.m5e)}`;
    d.beePrice.innerHTML = format(getBeePrice());
    if (tmp.totalTributes >= 10) {
        if (tmp.totalTributes >= 20)
            d.bees.innerHTML = `${p.bees} + ${format(tmp.totalTributes / 5)}`;
        else
            d.bees.innerHTML = `${p.bees} + ${format(tmp.totalTributes / 5, -1)}`;
    }
    else
        d.bees.innerHTML = "" + p.bees;
    ...
    d.donateToPollenGod.title = `1.03 ^ ${format(p.pollenGodTributes * tmp.m5e, 1)} = ${format(Math.pow(1.03, p.pollenGodTributes))}`;
    d.donateToNectarGod.title = `1.03 ^ ${format(p.nectarGodTributes * tmp.m5e, 1)} = ${format(Math.pow(1.03, p.nectarGodTributes))}`;
    d.donateToHoneyGod.title = `1.03 ^ ${format(p.honeyGodTributes * tmp.m5e, 1)} = ${format(Math.pow(1.03, p.honeyGodTributes))}`;
    d.donateToFlowerGod.title = `1.03 ^ ${format(p.flowerGodTributes * tmp.m5e, 1)} = ${format(Math.pow(1.03, p.flowerGodTributes))}`;
    d.donateToCapitalistGod.title = `1.03 ^ ${format(p.capitalistGodTributes * tmp.m5e, 1)} = ${format(Math.pow(1.03, p.capitalistGodTributes))}`;
    //
    if (tmp.totalTributes < tributeMilestones[0])
        d.m0e.innerHTML = "";
    else
    ...
    for (let i = 0; i < tributeMilestonesHide.length; i++) {
        if (!tmp.displayeverything && tributeMilestonesHide[i] > tmp.totalTributes)
